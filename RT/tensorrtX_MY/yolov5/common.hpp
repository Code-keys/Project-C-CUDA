...
std::map<std::string, Weights> loadWeights(const std::string file) {
    ...
    /*
    std::map<std::string, Weights>::iterator it;
    for(it = weightMap.begin(); it != weightMap.end(); it++){
        std::cout<<it->first<<":"<<it->second.count<<":"<<it->second.values+it->second.count<<std::endl;}
    */
    return weightMap;
}
...
ILayer* conv(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int outch, int ksize, int s, int g, std::string lname) {
    int p = ksize / 2;
    IConvolutionLayer* conv1 = network->addConvolutionNd(input, outch, DimsHW{ksize, ksize}, weightMap[lname + ".weight"], weightMap[lname + ".bias"]);
    assert(conv1);
    conv1->setStrideNd(DimsHW{s, s});
    conv1->setPaddingNd(DimsHW{p, p});
    conv1->setNbGroups(g);
